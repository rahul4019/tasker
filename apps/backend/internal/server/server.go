...
import (
	...
	loggerPkg "github.com/rahul4019/tasker/internal/logger"
...
func New(cfg *config.Config, logger *zerolog.Logger, loggerService *loggerPkg.LoggerService) (*Server, error) {
	...
	if err := redisClient.Ping(ctx).Err(); err != nil {
		logger.Error().Err(err).Msg("Faild to connect to Redis, continuing without Redis")
		// don't fail startup if Redis is unavailble
	}

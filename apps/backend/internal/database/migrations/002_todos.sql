...
CREATE UNIQUE INDEX idx_todo_categories_unique_name ON todo_categories (user_id, name);

CREATE TRIGGER set_updated_at_todo_categories BEFORE
UPDATE ON todo_categories FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at ();
...
-- Constraints
ALTER TABLE todos ADD CONSTRAINT no_self_parent CHECK (id != parent_todo_id);

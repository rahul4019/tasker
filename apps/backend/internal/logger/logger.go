...
func NewLoggerService(cfg *config.ObservabilityConfig) *LoggerService {
	...
	// Add debug logging if only explicitly enabled
...
// Shutdown shuts down the New Relic
...
// 	return NewLoggerService(&config.ObservabilityConfig{
...
func NewLoggerWithService(cfg *config.ObservabilityConfig, loggerService *LoggerService) zerolog.Logger {
	...
	if cfg.IsProduction() && cfg.Logging.Format == "json" {
		...
		// wrap with New Relic zerologgerWriter for log forwarding in production
	...
	logger := zerolog.New(writer).Level(loglevel).With().
		Timestamp().
	...
	writer := zerolog.ConsoleWriter{
		...
		FormatFieldValue: func(i any) string {
			...
			case string:
				...
				if len(v) > 200 {
					// truncate very long sql statement
					return v[:200] + "..."
				}
				return v
			case []byte:
